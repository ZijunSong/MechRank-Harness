...
LOG_DIR="${ROOT}/logs/pgllm_qwen38_27b_gpu4"
...
export PGLLM_REGISTRY="${PGLLM_REGISTRY:-$ROOT/configs/pgllm_qwen38_27b_vllm.json}"
export PGLLM_MODELS="${PGLLM_MODELS:-qwen38-27b}"
export PGLLM_SIZES="${PGLLM_SIZES:-50}"
export PGLLM_RUN_LABEL="${PGLLM_RUN_LABEL:-mechrank_qwen38_27b_direct_n50}"
export PGLLM_CONCURRENCY="${PGLLM_CONCURRENCY:-1}"
export PGLLM_TIMEOUT="${PGLLM_TIMEOUT:-900}"
...
echo "[$(date '+%F %T')] Starting vLLM on GPU${VLLM_GPU}, port ${VLLM_PORT}, max_model_len=${MAX_MODEL_LEN}" | tee -a "$VLLM_LOG"
...
echo "[$(date '+%F %T')] vLLM ready; launching PG-LLM bare-model eval" | tee -a "$PGLLM_LOG"
cd "$ROOT"
