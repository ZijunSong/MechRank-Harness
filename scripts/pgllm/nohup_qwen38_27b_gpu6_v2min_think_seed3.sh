...
# Seed 3 only, thinking-on H-min, same intensity as GPU0 seed 1 / bare think.
# Does not touch GPU0 (seed 1) or GPU3 (no-think leftover).
...
export PGLLM_SEEDS=3
export PGLLM_RUN_LABEL="mechrank_closer_v2min_think_qwen38_n50_b3"

export VLLM_GPU="${VLLM_GPU:-6}"
export VLLM_PORT="${VLLM_PORT:-8640}"
export CLOSER_PORT="${CLOSER_PORT:-8690}"
...
export GPU_MEM_HEADROOM_MIB="${GPU_MEM_HEADROOM_MIB:-4096}"
export MAX_MODEL_LEN="${MAX_MODEL_LEN:-131072}"
export MAX_NUM_SEQS="${MAX_NUM_SEQS:-1}"
export FROZEN_MANIFEST="$ROOT/artifacts/frozen/closer_v2min_think_qwen38_harness_b3_manifest.json"

mkdir -p "$LOG_DIR"
echo "[$(date '+%F %T')] GPU${VLLM_GPU} v2-min THINK seed=3 vllm=:${VLLM_PORT} closer=:${CLOSER_PORT} max_model_len=${MAX_MODEL_LEN}"
