...
from closer.llm.structured import (
    json_only_system,
    parse_model,
    repair_user_prompt,
)
...
class OpenAIResponsesClient(LLMClient):
    ...
    async def generate_structured(
        ...
        *,
        ...
        current_user = user
        extra_body = self._schema_body(schema) if self.config.structured_output.native_json_schema else None
        for attempt in range(attempts):
            try:
                ...
                if extra_body is not None and "response_format" in str(exc).lower() or (
                    extra_body is not None and "json_schema" in str(exc).lower()
                ):
                    extra_body = None
                    last_error = exc
                    continue
    ...
    def _schema_body(self, schema: type[BaseModel]) -> dict[str, Any]:
        from closer.llm.structured import model_json_schema
        ...
        return {
            "text": {
                "format": {
                    ...
                    "schema": model_json_schema(schema),
                    "strict": True,
                }
            }
        }
    ...
    async def _complete(
        ...
        *,
        ...
        if self.config.base_model.reasoning_effort:
            body["reasoning"] = {"effort": self.config.base_model.reasoning_effort}
        if extra_body:
        ...
        if payload.get("status") in {"failed", "incomplete"}:
            raise ProviderError(f"{stage}: incomplete or failed response: {payload.get('status')}")
        usage_raw = extract_usage(payload)
